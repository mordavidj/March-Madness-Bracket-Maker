...
teams_df = teams_df[teams_df['LastD1Season'] == 2023]

elo_df = teams_df.assign(eloRating=INITIAL_ELO_RATING)[['eloRating']]
elo_df['eloRating'] = INITIAL_ELO_RATING

print(elo_df)

reg_season_df = pd.read_csv("data_2023/MRegularSeasonCompactResults.csv.zip", compression='zip')
reg_season_df = reg_season_df[reg_season_df['Season'] == 2023]
print(reg_season_df)
...
for index, row in reg_season_df.iterrows():
    ...
    w_elo = elo_df.at[row['WTeamID'], 'eloRating']
    # print(f'w_elo: {w_elo}')
    # l_ind = elo_df.index[elo_df['TeamID'] == row['LTeamID']]
    l_elo = elo_df.at[row['LTeamID'], 'eloRating']
    # print(f'l_elo: {l_elo}')
    ...
    elo_df.at[row['WTeamID'], 'eloRating'] = w_elo + d_elo
    elo_df.at[row['LTeamID'], 'eloRating'] = l_elo - d_elo
